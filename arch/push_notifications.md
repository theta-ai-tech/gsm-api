# Plan — Push Notification Delivery (option b)
...
**Child issues:** PUSH-1 … PUSH-7 (label `push-notifications`).
...
without re-deriving the architecture. No code in this doc — it is the blueprint.
...
(`api/app/repos/notification_intent_repo.py` → `users/{uid}/notificationIntents/{intentId}`). But
**nothing delivers those intents** — no device token is stored on the user, and no code calls
...
APNs/FCM token and POSTing it up). This doc + the PUSH-1…7 issues exist so that when iOS is ready, a
...
```
 ...
 └──────────────┬──────────────────────────────┘
                │ @on_document_created  (NEW — PUSH-4)
                ▼
 ┌─────────────────────────────────────────────┐
...
Until iOS ships steps 1–3, `deviceTokens` is always empty and the trigger is a no-op. **This is the
...
| --- | --- | --- | --- |
| **PUSH-1** | Device token storage: model field, onboarding init, `UsersRepo` CRUD, dedupe | `models/user.py`, `services/onboarding_service.py`, `repos/users_repo.py`, `wiki/DATA_DICTIONARY.md` | — |
...
| **PUSH-4** | `@on_document_created` trigger → deliver via FCM; wire in `functions/main.py` | `functions/notification_triggers/`, `functions/main.py`, `wiki/functions.md` | PUSH-1, PUSH-3 |
...
| **PUSH-7** | Deploy wiring + ops runbook + flip wiki note to "live" | `scripts/deploy_functions.sh`, `scripts/smoke_triggers.sh`, `wiki/notifications.md`, operator playbook | PUSH-1…6 |
...
- `wiki/notifications.md` — the intent contract (fields, types, emission points) + planned-delivery
  note.
