...
|------|-----------|-------|
| `osm_courts_raw.json` | `tools/fetch_courts_osm.py` | List of raw candidates: `name`, `lat`, `lng`, `sports[]`, `osm_type`, `osm_id`, `courts`, `surface`, `building` |
...
2. **Normalise** them into the review checkpoint:
   ...
   re-running the pipeline upserts in place and never duplicates.
...
3. **Review & edit** `venue_checkpoint.json` by hand. It is indented and sorted
   ...
   - delete whole rows for venues that should not be seeded.
