#!/usr/bin/env bash
# Smoke tests for PR #330: INT-1 reject singles result self-confirmation (#319)
# Generated: 2026-06-01
# Usage: API_BASE_URL=http://127.0.0.1:8330 bash tests/smoke/pr-330.sh
#
# Requires: make emu-all running. This script starts the PR API itself.
#
# Verifies the singles self-confirmation guard:
#   - The player who SUBMITTED a singles result cannot also CONFIRM it.
#     A second verify-score call from the original submitter is rejected
#     with 409 "Confirmation must come from the opposing player".
#   - The OPPOSING participant confirming the same result still completes
#     the match (200, status completed) — genuine two-party agreement works.
...
# ═══════════════════════════════════════════════════════════════════════════════
echo ""
echo "── Section 1: submitter cannot self-confirm; opposing player can ───────────"
# ═══════════════════════════════════════════════════════════════════════════════
...
# Iggy submits score (first submission) → pending_confirmation, Iggy recorded as submitter
...
# Iggy tries to CONFIRM his own result → must be rejected with 409 (the fix)
BODY_FILE=$(mktemp)
SELF_CODE=$(curl -s -o "$BODY_FILE" -w "%{http_code}" \
  -X POST "$API/matches/$MATCH_ID/verify-score" \
  -H "Authorization: Bearer $IGGY" \
  -H "Content-Type: application/json" \
  -d '{"winner_uid": "user_ignatios"}')
SELF_DETAIL=$(jq -r '.detail // empty' "$BODY_FILE")
rm -f "$BODY_FILE"
assert_eq "Iggy self-confirm rejected with 409" "$SELF_CODE" "409"
assert_eq "409 detail mentions opposing player" "$SELF_DETAIL" "Confirmation must come from the opposing player"

# Match must still be pending_confirmation (self-confirm did NOT complete it)
MATCH_DOC=$(curl -s "$FIRESTORE/matches/$MATCH_ID")
assert_eq "Match still pending_confirmation after self-confirm" \
  "$(echo "$MATCH_DOC" | jq -r '.fields.status.stringValue // empty')" "pending_confirmation"

# Alice (opposing player) confirms the same result → completes normally
SCORE2=$(curl -s -X POST "$API/matches/$MATCH_ID/verify-score" \
  -H "Authorization: Bearer $ALICE" \
  -H "Content-Type: application/json" \
  -d '{"winner_uid": "user_ignatios"}')
assert_eq "Alice (opposing) confirms → completed" \
...
# ── Summary ───────────────────────────────────────────────────────────────────
echo ""
echo "Smoke tests PR #330: $PASS passed, $FAIL failed"
